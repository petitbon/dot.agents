...
| Proportional design, release integrity, booking/Scheduling routing, source precedence, realtime current-state, prompt-channel, and Mermaid-render guidance must not regress. | Owning skill plus `repo-agent-governance` | Affected skill and reference files | `./scripts/validate-skills.sh`, `./scripts/test-validate-skills.sh` | Hard for encoded guards; manual review for other semantics |
